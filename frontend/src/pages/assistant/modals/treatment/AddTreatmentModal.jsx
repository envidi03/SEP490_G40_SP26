...
const AddTreatmentModal = ({ isOpen, onClose, record, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const implicitAppointmentId = record.appointment_id;

      // 1. KIỂM TRA LỖI TRƯỚC KHI GỬI (Validation nội bộ)
      for (let i = 0; i < forms.length; i++) {
        const form = forms[i];
        if (!form.tooth_position.trim()) {
          errors.push(`Phiếu #${i + 1}: Vui lòng nhập vị trí răng điều trị.`);
        }
        if (form.phase === "SESSION" && !implicitAppointmentId) {
          errors.push(`Phiếu #${i + 1}: Không tìm thấy Mã lịch hẹn cho ca Thực thi (Vui lòng kiểm tra lại Bệnh án).`);
        }
      }

      // Nếu mảng errors có lỗi -> Hiển thị Popup lỗi và dừng lại
      if (errors.length > 0) {
      ...
      for (let i = 0; i < forms.length; i++) {
        ...
        try {
            ...
            payload.appointment_id = implicitAppointmentId;
            }
            ...
            // Nếu có 1 form bị lỗi API (ví dụ server báo lỗi), gom luôn vào mảng lỗi
            console.error("API Error ở phiếu", i+1, apiError);
            const errorMsg = apiError.response?.data?.message || apiError.message || "Lỗi hệ thống không xác định.";
            errors.push(`Lỗi máy chủ tại Phiếu #${i + 1}: ${errorMsg}`);
        }
      }

      // 3. XỬ LÝ KẾT QUẢ SAU KHI CHẠY XONG VÒNG LẶP
      if (errors.length > 0) {
         // Nếu chạy hết vòng lặp mà có lỗi (gửi đc 1 vài cái, lỗi 1 vài cái)
         setErrorPopup({ isOpen: true, messages: errors });
         // Vẫn load lại list ngoài cha để lấy những phiếu đã lưu thành công
         if (successCount > 0) onSuccess(); 
      } else {
         // Tất cả thành công mĩ mãn
         onSuccess();
         onClose();
      }
      ...
      setErrorPopup({ isOpen: true, messages: ["Đã có lỗi hệ thống xảy ra, vui lòng thử lại."] });
    } finally {
      setLoading(false);
    }
  };
        ...
        >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-slate-500 mt-0.5">
                  Hồ sơ: <span className="text-blue-600">{record.record_name}</span>
                </p>
              </div>
            </div>
            <button
          ...
          <div className="p-6 overflow-y-auto max-h-[65vh] custom-scrollbar space-y-6">
            {forms.map((form, index) => (
              <div key={form._localId} className="relative p-6 bg-slate-50 border-2 border-slate-100 rounded-2xl shadow-sm">
                ...
                {/* Hàng 2: Thời gian, Tiền, Số lượng */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                  <div className="space-y-2">
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">
                      {form.phase === "PLAN" ? "Ngày dự kiến" : "Ngày thực hiện"}
                    </label>
                    <input
                      type="date"
                      name="planned_date"
          ...
          <div className="p-6 border-t border-slate-100 bg-slate-50 flex items-center justify-between rounded-b-3xl">
            <p className="text-sm font-bold text-slate-400">
              Tổng cộng: <span className="text-blue-600 text-xl mx-1">{forms.length}</span> phiếu
            </p>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-200">
           <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
              <div className="p-6 text-center border-b border-slate-100">
                  ...
                  <h3 className="text-xl font-black text-slate-800 tracking-tight">Opps! Đã có lỗi xảy ra</h3>
                  <p className="text-sm font-medium text-slate-500 mt-2">Hệ thống phát hiện một vài vấn đề trong phiếu điều trị của bạn.</p>
              </div>
