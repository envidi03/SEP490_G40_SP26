...
exports.dispensePrescription = async (treatmentId) => {
    try {
        ...
        for (const item of treatment.medicine_usage) {
            ...
            if (updatedMedicine && updatedMedicine.quantity <= updatedMedicine.min_quantity) {
                try {
                    ...
                    logger.error(`Lỗi gửi cảnh báo LOW STOCK: ${err.message}`);
                }
            }
        ...
        if (error.statusCode) {
            logger.warn(`Business logic error in dispensePrescription: ${error.message}`);
        } else {
            logger.error(`Error in dispensePrescription: ${error.message}`);
        }
        throw error;
    }
};
