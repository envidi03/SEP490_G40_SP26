...
exports.getMedicines = async ({ page = 1, limit = 10, search, category, statusFilter }) => {
    try {
        ...
        logger.error(`Error in getMedicines: ${error.message}`);
        throw error;
    }
};
...
exports.createMedicine = async (data) => {
    try {
        ...
        if (error.statusCode) {
            logger.warn(`Business logic error in createMedicine: ${error.message}`);
        } else {
            logger.error(`Error in createMedicine: ${error.message}`);
        }
        throw error;
    }
};
...
exports.updateMedicine = async (id, data) => {
    try {
        ...
        if (error.statusCode) {
            logger.warn(`Business logic error in updateMedicine: ${error.message}`);
        } else {
            logger.error(`Error in updateMedicine: ${error.message}`);
        }
        throw error;
    }
};
...
exports.getMedicineById = async (id) => {
    try {
        ...
        if (error.statusCode) {
            logger.warn(`Business logic error in getMedicineById: ${error.message}`);
        } else {
            logger.error(`Error in getMedicineById: ${error.message}`);
        }
        throw error;
    }
};
...
exports.createRestockRequest = async (medicineId, data) => {
    try {
        ...
        try {
            ...
            logger.error(`Lỗi gửi thông báo RESTOCK_REQUESTED: ${err.message}`);
        }
        ...
        if (error.statusCode) {
            logger.warn(`Business logic error in createRestockRequest: ${error.message}`);
        } else {
            logger.error(`Error in createRestockRequest: ${error.message}`);
        }
        throw error;
    }
};
...
exports.getRestockRequests = async ({ status, priority, search, page = 1, limit = 10 }) => {
    try {
        ...
        logger.error(`Error in getRestockRequests: ${error.message}`);
        throw error;
    }
};
